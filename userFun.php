...
function uploadUserImage($page){
    if(isset($_FILES['userImg'])){
        ...
        if($error) redirect($page."?FileError=" .  $error);
        if($_FILES['userImg']["size"]==0){
            redirect($page."?error=" ."fill all spaces");
        }else{
            $img=$_FILES['userImg'];
            $temp=$img['tmp_name'];
...
function update(){
    if(isset($_POST["Fname"]) && isset($_POST["city"])&& isset($_POST["userImg"]) &&
    ...
    else redirect("updateUserInfo.php?error=" .  "make sure to fill all spaces  "."&userEmail=".$_GET["userEmail"]);
